<?php
class Registrar_Adapter_Az extends Registrar_AdapterAbstract
{
    public $config = array(
        'user'    =>  null,
        'klucz'  =>  null
    ...
    public function __construct($options)
    {
        ...
        if(isset($options['user']) && !empty($options['user'])) {
            ...
            throw new Registrar_Exception('Rejestrator nie jest skonfigurowany proszę uzupełnić pole Az User w "Konfiguracja -> Rejestrator Domen".');
        }

        if(isset($options['klucz']) && !empty($options['klucz'])) {
            $this->config['klucz'] = $options['klucz'];
            unset($options['klucz']);
        } else {
            throw new Registrar_Exception('Rejestrator nie jest skonfigurowany proszę uzupełnić pole Az klucz w "Konfiguracja -> Rejestrator Domen".');
        }
    }
    ...
    {
        return array(
            'label'     =>  'Manages domains on Az via API',
            'form'  => array(
                'user' => array('text', array(
                            'label' => 'Az Username', 
                            'description'=>'Az Username'
                        ),
                     ),
                'klucz' => array('text', array(
                            'label' => 'Az klucz', 
                            'description'=>'Az klucz',
                            'renderPassword'    =>  true, 
                        ),
                     ),
            ),
    ...
    public function isDomainAvailable(Registrar_Domain $domain)
    {
		...
		$pass = "".$this->config['klucz']."";
		$urlDomain = 'http://panel.az.pl:9080/produkcyjny/api/domain?wsdl';
		$client = new SoapClient($urlDomain,
		 array(
		 'cache_wsdl' => "WSDL_CACHE_NONE",
		 'features'   => SOAP_SINGLE_ELEMENT_ARRAYS,
		 ));	
		//sprawdzanie dostepnosci domeny, array('features' => SOAP_SINGLE_ELEMENT_ARRAYS)
		$paramss = new stdClass;
		$paramss->auth->pass = $pass;
		$paramss->auth->uid = $uid;
		$paramss->domains = array("{$domain->getSld()}{$domain->getTld()}");
		$resp = $client->domainsCheck($paramss);

        if ($resp->return->domainCheck[0]->status == 1) return true;
        else return false;
    }
    ...
    public function isDomainCanBeTransfered(Registrar_Domain $domain)
    {
		...
		$uid = $this->config['user'];
		$pass = "".$this->config['klucz']."";
		$urlDomain = 'http://panel.az.pl:9080/produkcyjny/api/domainTransfer?wsdl';
		$client = new SoapClient($urlDomain,
		 array(
		 'cache_wsdl' => "WSDL_CACHE_NONE",
		 'features'   => SOAP_SINGLE_ELEMENT_ARRAYS,
		...
		$paramss->auth->pass = $pass;
		$paramss->auth->uid = $uid;
		$paramss->DomainTransferInfoDataIN->domain = $domain->getSld().$domain->getTld();
		$resp = $client->transferInfo($paramss);
		print_r($resp);
    }
    ...
    public function modifyNs(Registrar_Domain $domain)
    {
		...
		$paramss->auth->uid = $uid;
		$paramss->domains = array("{$domain->getSld()}{$domain->getTld()}");
		$paramss->domainDelegChangeDataIN = array(
		...
		$resp = $client->delegChange($paramss);
    ...
    public function getDomainDetails(Registrar_Domain $domain)
    {
        ...
        {
            throw new Registrar_Exception(sprintf('AzApiError: "%s"', implode(', ', $result->errors)), 100);
        }
    ...
    public function transferDomain(Registrar_Domain $domain)
    {
        ...
        if ($this->_getDomainLock($domain)) {
        	if (!$this->_toggleDomainLock($domain, 1)) {
        		throw new Exception('AzApiError: Can\'t unlock domain for transfer');
        	}
        }
        $result = $this->_makeRequest($params);
    ...
    {
        if($this->isTestEnv()) {
            return 'http://resellertest.Az.com/interface.asp';
        }
        return 'http://reseller.Az.com/interface.asp';
    }
    ...
    private function _parseResponse($result, $type = 'xml')
    {
        ...
        if (isset($xml->ErrCount) && $xml->ErrCount > 0) {
            throw new Registrar_Exception(sprintf('AzApiError: "%s"', implode(', ', (array)$xml->errors)), 100);
        }
...
//                throw new Registrar_Exception(sprintf('Required param "%s" can not be blank', $param));
    ...
    /**
     * 
     * Moves domain to another Az account
    ...
    /**
     * 
     * Generates Az friendly username from email
     * @param string $email
     * @return string
     */
    ...
    private function _checkPrivacy(Registrar_Domain $domain) {
        ...
        if (isset($result->GetWPPSInfo)) {
        	...
        	throw new Registrar_Exception('AzApiError: Can\'t get domain privacy information');
        }
    }
