...
class Registrar_Adapter_Ovh extends Registrar_AdapterAbstract
{
    ...
    public function __construct($options)
    {
        ...
        if(isset($options['user']) && !empty($options['user'])) {
            ...
            throw new Registrar_Exception('Rejestrator nie jest skonfigurowany proszę uzupełnić pole Ovh User w "Konfiguracja -> Rejestrator Domen".');
        }
        ...
        if(isset($options['password']) && !empty($options['password'])) {
            ...
            throw new Registrar_Exception('Rejestrator nie jest skonfigurowany proszę uzupełnić pole Ovh Password w "Konfiguracja -> Rejestrator Domen".');
        }
    }
    ...
    public function isDomainAvailable(Registrar_Domain $domain)
    {
		 ...
		 //login
		 $session = $soap->login($this->config['user'], $this->config['password'],"pl", false);
		
		 //domainCheck
		 $result = $soap->domainCheck($session, $domain->getSld().$domain->getTld());
		 
		 if ($result[0]->value == 1) return true;
        else return false;
    ...
    public function isDomainCanBeTransfered(Registrar_Domain $domain)
    {
		 ...
		 //domainCheck
		 $result = $soap->domainCheck($session, $domain->getSld().$domain->getTld());
		 if ($result[1]->value == 1) return true;
        else return false;
    ...
    public function modifyNs(Registrar_Domain $domain)
    {
        ...
        foreach($domain->getNameservers() as $nse) {
		 ...
		 $result = $soap->domainDnsUpdate($session, $params['domain'], $params['NS1'], "", $params['NS2'], "", $params['NS3'], "", $params['NS4'], "", "", "");
    ...
    public function getDomainDetails(Registrar_Domain $domain)
    {
        ...
        {
            throw new Registrar_Exception(sprintf('OvhApiError: "%s"', implode(', ', $result->errors)), 100);
        }
    ...
    public function transferDomain(Registrar_Domain $domain)
    {
        ...
        if ($this->_getDomainLock($domain)) {
        	if (!$this->_toggleDomainLock($domain, 1)) {
        		throw new Exception('OvhApiError: Can\'t unlock domain for transfer');
        	}
        }
        $result = $this->_makeRequest($params);
    ...
    public function enablePrivacyProtection(Registrar_Domain $domain)
    {
        $soap = new SoapClient("https://www.ovh.com/soapi/soapi-re-1.59.wsdl");
		 $session = $soap->login($this->config['user'], $this->config['password'],"pl", false);
		 $result = $soap->domainWhoisObfuscatorSet($session, $domain->getSld().$domain->getTld(), "ALL");
    ...
    public function disablePrivacyProtection(Registrar_Domain $domain)
    {
         $soap = new SoapClient("https://www.ovh.com/soapi/soapi-re-1.59.wsdl");
		 $session = $soap->login($this->config['user'], $this->config['password'],"pl", false);
		 $result = $soap->domainWhoisObfuscatorUnset($session, $domain->getSld().$domain->getTld(), "ALL");
    ...
    public function lock(Registrar_Domain $domain)
    {
		 ...
		 //domainCheck
		 $result = $soap->domainLock($session, $domain->getSld().$domain->getTld());
    ...
    public function unlock(Registrar_Domain $domain)
    {
		 ...
		 //domainCheck
		 $result = $soap->domainUnlock($session, $domain->getSld().$domain->getTld());
    ...
    {
        if($this->isTestEnv()) {
            return 'http://resellertest.Ovh.com/interface.asp';
        }
        return 'http://reseller.Ovh.com/interface.asp';
    }
    ...
    private function _parseResponse($result, $type = 'xml')
    {
        ...
        if (isset($xml->ErrCount) && $xml->ErrCount > 0) {
            throw new Registrar_Exception(sprintf('OvhApiError: "%s"', implode(', ', (array)$xml->errors)), 100);
        }
...
//                throw new Registrar_Exception(sprintf('Required param "%s" can not be blank', $param));
    ...
    /**
     * 
     * Moves domain to another Ovh account
    ...
    /**
     * 
     * Generates Ovh friendly username from email
     * @param string $email
     * @return string
     */
    ...
    private function _checkPrivacy(Registrar_Domain $domain) {
        ...
        if (isset($result->GetWPPSInfo)) {
        	...
        	throw new Registrar_Exception('OvhApiError: Can\'t get domain privacy information');
        }
    }
